...
def evaluate_predictions(
    ...
    metrics: Dict[str, float] = {
        ...
        "gmean_abs_error": safe_gmean_abs_error(abs_err),
    }
    ...
    for g in ("many", "medium", "few"):
        ...
        if len(idxs) == 0:
            metrics[f"{g}_mae"] = float("nan")
            metrics[f"{g}_rmse"] = float("nan")
        else:
            yt = y_true[idxs]
            yp = y_pred[idxs]
            metrics[f"{g}_mae"] = float(mean_absolute_error(yt, yp))
            metrics[f"{g}_rmse"] = float(math.sqrt(mean_squared_error(yt, yp)))

    low_thr = np.percentile(y_true, 10)
    high_thr = np.percentile(y_true, 90)
    low_mask = y_true <= low_thr
    high_mask = y_true >= high_thr
    metrics["low10_mae"] = float(mean_absolute_error(y_true[low_mask], y_pred[low_mask])) if np.any(low_mask) else float("nan")
    metrics["high10_mae"] = float(mean_absolute_error(y_true[high_mask], y_pred[high_mask])) if np.any(high_mask) else float("nan")

    cold_mask = y_true <= cold_threshold
    hot_mask = y_true >= hot_threshold
    metrics["cold_tail_mae"] = float(mean_absolute_error(y_true[cold_mask], y_pred[cold_mask])) if np.any(cold_mask) else float("nan")
    metrics["hot_tail_mae"] = float(mean_absolute_error(y_true[hot_mask], y_pred[hot_mask])) if np.any(hot_mask) else float("nan")
    ...
    if cam_ids is not None:
        ...
        metrics["camera_mae_mean"] = float(np.mean(cam_maes)) if cam_maes else float("nan")
        metrics["camera_mae_std"] = float(np.std(cam_maes)) if cam_maes else float("nan")
    else:
        metrics["camera_mae_mean"] = float("nan")
        metrics["camera_mae_std"] = float("nan")

    metrics.update({f"val_{k}_count": int(v) for k, v in summarize_shot_labels(eval_buckets, shot_map).items()})
    metrics["train_eval_bucket_nonempty"] = int(np.sum(train_eval_bucket_counts > 0))
    metrics["train_eval_bucket_empty"] = int(np.sum(train_eval_bucket_counts == 0))
    return metrics
...
def train(args: argparse.Namespace) -> RunArtifacts:
    ...
    train_idx, val_idx = split_indices(df, args.split_mode, args.val_ratio, args.seed, group_col=args.group_col)
    ...
    return RunArtifacts(run_dir=run_dir, best_metrics=best_metrics)
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--split_mode", type=str, default="cross_camera", choices=["cross_camera", "random"])
